...
public:
    explicit GameWidget(QWidget* parent = nullptr) : QWidget(parent), background("test.png"), boss(nullptr) {
        ...
        animPlayer = new Animation("../resource/Player/run_r/%1.png", "../img_boss_hit/%1.png", 6, 14, IMG_INTERVAL, 2);
...
private slots:
    void updateGame() {
        ...
        boss.move(player);
        player->checkHurt(&boss);

        update();
    }
};

void Player::checkHurt(Boss* boss)
{
    float dx = position.x - boss->position.x;
    float dy = position.y - boss->position.y;
    float dist = std::sqrt(dx*dx + dy*dy);

    if (boss->isAttacking && dist < 300)
    {
        currentBlood -= 10;
        isHit = true;
        hitFrameCount = 14;
